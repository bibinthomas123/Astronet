...
def _process_tce(feature_config):
  ...
  if "global_view" in feature_config:
      ...
      features["global_view"] = np.expand_dims(global_view.astype(np.float32), 0)

  if "local_view" in feature_config:
      local_view = preprocess.local_view(time, flux, FLAGS.period, FLAGS.duration)
      features["local_view"] = np.expand_dims(local_view.astype(np.float32), 0)
...
def main(_):
  ...
  for prediction in estimator.predict(input_fn):
    ...
    if FLAGS.output_prediction_file:
        with open(FLAGS.output_prediction_file, "w") as f:
            try:
                  f.write(f"{float(prediction):.6f}\n")  # Works for scalar
            except:
                  f.write(f"{float(prediction[0]):.6f}\n")  # If it's a 1D array
